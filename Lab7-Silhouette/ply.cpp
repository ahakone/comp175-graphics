...
void ply::findEdges(){
  ...
  edgeCount = 0;
  // assumes facelist and vertexlist != NULL
  edgeList[0].vertices[0] = faceList[0].vertexList[0];
  edgeList[0].vertices[1] = faceList[0].vertexList[1];
  edgeList[0].faces[0] = 0;
  edgeCount++;
  ...
  for (int i = 0; i < faceCount; i++) {
    for (int j = 0; j < 3; j++) {
      ...
      for (k = 0; k < edgeCount; k++) {
        if ((faceList[i].vertexList[j] == edgeList[k].vertices[0] &&
             faceList[i].vertexList[(j+1)%3] == edgeList[k].vertices[1]) ||
            (faceList[i].vertexList[(j+1)%3] == edgeList[k].vertices[0] &&
             faceList[i].vertexList[j] == edgeList[k].vertices[1]) ){
          if (edgeCount != 1) {
            edgeList[k].faces[1] = i;
          }
          break;
          //continue;
        }
      }
      if (edgeList[k].faces[0] != 0 && k == edgeCount) {
        edgeList[edgeCount].faces[0] = i;
        edgeList[edgeCount].vertices[0] = faceList[i].vertexList[j];
        edgeList[edgeCount].vertices[1] = faceList[i].vertexList[(j+1)%3];
        edgeCount++;
      }
    }
  }
...
void ply::renderSilhouette(){
  Vector look(lookX, 1, lookZ);
  for (int i = 0; i < edgeCount; i++) {
    if ((dot(look, faceList[edgeList[i].faces[0]].getNorm()) > 0 && 
         dot(look, faceList[edgeList[i].faces[1]].getNorm()) > 0) ||
        (dot(look, faceList[edgeList[i].faces[0]].getNorm()) < 0 && 
         dot(look, faceList[edgeList[i].faces[1]].getNorm()) < 0)) {
      // do nothing
    }
      ...
      glPushMatrix();
      glBegin(GL_LINES);
      glVertex3f(vertexList[edgeList[i].vertices[0]].x, 
                 vertexList[edgeList[i].vertices[0]].y, 
                 vertexList[edgeList[i].vertices[0]].z);
      glVertex3f(vertexList[edgeList[i].vertices[1]].x, 
                 vertexList[edgeList[i].vertices[1]].y, 
                 vertexList[edgeList[i].vertices[1]].z);
      glEnd();
      glPopMatrix();
    }
  }
