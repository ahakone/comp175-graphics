...
void ply::scaleAndCenter() {
	...
	float x_max = vertexList[0].x;
	float y_max = vertexList[0].y;
	float z_max = vertexList[0].z;
	float x_min = x_max;
	float y_min = y_max;
	float z_min = z_max;
	float x_mid;
	...
	// find min and max for all axes
	for (i = 0; i < vertexCount; i++) {
		if (x_max < vertexList[i].x) {
			x_max = vertexList[i].x;
		}
		if (x_min > vertexList[i].x) {
			x_min = vertexList[i].x;
		}

		if (y_max < vertexList[i].y) {
			y_max = vertexList[i].y;
		}
		if (y_min > vertexList[i].y) {
			y_min = vertexList[i].y;
		}

		if (z_max < vertexList[i].z) {
			z_max = vertexList[i].z;
		}
		if (z_min > vertexList[i].z) {
			z_min = vertexList[i].z;
		}
	}
	...
	xscale = .5 / (x_max - x_mid);
	yscale = .5 / (y_max - y_mid);
	zscale = .5 / (z_max - z_mid);

	// centers vertexList[i] - midpoint
	// then scales
	for (i = 0; i < vertexCount; i++) {
		vertexList[i].x = (vertexList[i].x - x_mid) * xscale;
		vertexList[i].y = (vertexList[i].y - y_mid) * yscale;
		vertexList[i].z = (vertexList[i].z - z_mid) * zscale;
	}
...
/*  ===============================================
	...
	*/
		...
		for (int i = 0; i < faceCount; i++) {
			face f = faceList[i];
			int *vindList = faceList[i].vertexList;
			vertex v1 = vertexList[vindList[0]];
			vertex v2 = vertexList[vindList[1]];
			vertex v3 = vertexList[vindList[2]];
