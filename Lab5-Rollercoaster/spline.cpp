...
void spline::draw_spline(int resolution, int output){
	...
	while (iter->next != NULL) {
		for (t = 0; t < resolution; t++) {
			computed_x_y_z[index] = t / resolution + iter->x;
			computed_x_y_z[index + 1] = calculate_Spline(t / resolution, iter->y, iter->next->y, VELOCITY, VELOCITY );
			computed_x_y_z[index + 2] = calculate_Spline(t / resolution, iter->z, iter->next->z, VELOCITY, VELOCITY );
			glBegin(GL_POINTS);
				glVertex3f(computed_x_y_z[index], computed_x_y_z[index + 1], computed_x_y_z[index + 2]);
			glEnd();

			if (output) {
				//printf("x: %f y: %f z: %f\n", computed_x_y_z[index], computed_x_y_z[index + 1], computed_x_y_z[index + 2]);
			}
			index += 3;
		}
		iter = iter->next;
