...
Matrix Camera::GetProjectionMatrix() {
	...
	float theta_w = viewAngle * screenWidth / screenHeight;
	//scale
	Matrix normalize( 1/ (tan(theta_w/2) * farPlane), 0, 0, 0,
					  0, 1/ (tan(viewAngle/2) * farPlane), 0, 0,
					  0, 0, 1/farPlane, 0,
					  0,0,0,1);

	Matrix m = unhinge * normalize;
	return m;
}
