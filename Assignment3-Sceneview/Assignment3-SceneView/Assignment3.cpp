...
void myGlutDisplay(void)
{
  ...
  if (wireframe) {
    ...
    int i;
    for (i = 0; i < flatTree.size(); i++) {
      glPushMatrix();
          glMultMatrixd(flatTree[i]->GetTMatrix().unpack());
          renderShape(flatTree[i]->GetPrimitive().type);
      glPopMatrix();
    }
  }
  ...
  if (fillObj == 1) {
    ...
    int i;
    for (i = 0; i < flatTree.size(); i++) {
      applyMaterial(flatTree[i]->GetPrimitive().material);
      glPushMatrix();
          glMultMatrixd(flatTree[i]->GetTMatrix().unpack());
          renderShape(flatTree[i]->GetPrimitive().type);
      glPopMatrix();
...
int main(int argc, char* argv[])
{
  ...
  glui->add_button("Reset Camera", RESET_ID, callback_button);
